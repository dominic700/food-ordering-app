...
// Used by the bot to decide what welcome message + button to show.
export async function detectRole(telegramId) {
  ...
  // 4. New user — not registered yet
  return { role: 'new', account: null };
}


// ── saveCustomer ───────────────────────────────────────────────
// Creates a global_account for a new customer using the phone
// number they share via the Telegram contact button.
export async function saveCustomer(telegramId, name, phone) {
  // Upsert — safe to call even if they already exist
