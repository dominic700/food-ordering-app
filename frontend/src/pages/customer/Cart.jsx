...
export default function Cart() {
  ...
  // Totals — mirrors the backend's calculation exactly
  const subtotal      = cart.reduce((s, i) => s + i.base_price * i.quantity, 0);
  const feeTotal       = cart.reduce((s, i) => s + i.service_fee * i.quantity, 0);
  ...
  const total = paymentMethod === 'transfer' ? listTotal : walletTotal;

  async function handlePlaceOrder() {
    if (cart.length === 0) return;

    if (paymentMethod === 'wallet' && !canAffordWallet) {
      return telegram.alert('Insufficient balance and credit for this order.');
    }
    ...
    try {
      ...
      const paymentInfo = paymentMethod === 'transfer'
        ? { payment_method: 'transfer', transfer_provider: transferProvider, transaction_number: transactionNumber.trim() }
  ...
  if (success) {
    return (
      <div className="page" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', textAlign: 'center' }}>
        ...
        <div style={{ color: 'var(--text2)', fontSize: 14, marginBottom: 8, padding: '0 32px' }}>
          {paymentMethod === 'transfer'
            ? "We'll verify your transfer and the cafe will prepare your order."
        ...
        <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--red)', fontSize: 22, fontWeight: 800, marginBottom: 32 }}>
          {parseFloat(success.order.total).toFixed(2)} ETB
        </div>
      ...
      <div style={{ padding: '16px 16px 0' }}>
          ...
          {cart.map(item => (
            <div key={item.menu_item_id} className="cart-item">
              ...
              <div className="cart-item-info">
                ...
                <div className="cart-item-price">{item.price.toFixed(2)} ETB</div>
                {item.discount_percent > 0 && (
                  <div style={{ fontSize: 11, color: 'var(--green)' }}>{item.discount_percent}% off applied</div>
                )}
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14, color: 'var(--text2)' }}>
            <span>Subtotal</span><span style={{ fontFamily: 'var(--font-mono)' }}>{subtotal.toFixed(2)} ETB</span>
          </div>
          {feeTotal > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14, color: 'var(--text2)' }}>
              <span>Service Fee</span><span style={{ fontFamily: 'var(--font-mono)' }}>{feeTotal.toFixed(2)} ETB</span>
            </div>
          )}
          {discountTotal > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14, color: 'var(--green)' }}>
              <span>Discount{paymentMethod === 'transfer' ? ' (wallet only)' : ''}</span>
              <span style={{ fontFamily: 'var(--font-mono)' }}>
                {paymentMethod === 'transfer' ? '0.00' : `-${discountTotal.toFixed(2)}`} ETB
              </span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 17 }}>
            <span>Total</span>
            <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--red)' }}>{total.toFixed(2)} ETB</span>
          </div>
        </div>
        ...
        <div className={`payment-option ${paymentMethod === 'wallet' ? 'selected' : ''}`} onClick={() => setPaymentMethod('wallet')}>
          ...
          <div className="payment-info">
            ...
            <div className="payment-desc">
              Balance: {balance.toFixed(2)} ETB · Credit available: {creditAvail.toFixed(2)} ETB
            </div>
          </div>
          <span className={`badge ${canAffordWallet ? 'badge-sufficient' : 'badge-cancelled'}`}>
            {canAffordWallet ? 'Sufficient' : 'Insufficient'}
          </span>
        </div>
        ...
        <div
          ...
          style={{ marginBottom: paymentMethod === 'transfer' ? 0 : 10 }}
        >
          ...
          <div className="payment-info">
            ...
            <div className="payment-desc">Telebirr, CBE Birr, or Bank Transfer — full price, no discount</div>
          </div>
        </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div className="input-group">
              ...
              <select className="input" style={{ paddingLeft: 14 }} value={transferProvider} onChange={e => setTransferProvider(e.target.value)}>
                {TRANSFER_PROVIDERS.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
              </select>
            </div>
            <div className="input-group" style={{ marginBottom: 0 }}>
              <label className="input-label">Transaction Number</label>
              <input className="input" style={{ paddingLeft: 14 }} placeholder="e.g. 1234567890" value={transactionNumber} onChange={e => setTransactionNumber(e.target.value)} />
            </div>
          </div>
        )}
        ...
        <div className="card" style={{ marginBottom: 16, fontSize: 13 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)' }}>{total.toFixed(2)} ETB</span>
          </div>
          {paymentMethod === 'wallet' && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ color: 'var(--text2)' }}>From Balance</span>
                <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--green)' }}>{Math.min(balance, total).toFixed(2)} ETB</span>
              </div>
              {total > balance && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ color: 'var(--text2)' }}>From Credit</span>
                  <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--red)' }}>{Math.min(total - balance, creditAvail).toFixed(2)} ETB</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700 }}>
                <span>Remaining Balance</span>
                <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--green)' }}>{Math.max(balance - total, 0).toFixed(2)} ETB</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="input-group">
          ...
          <input className="input" style={{ paddingLeft: 14 }} placeholder="Any special requests..." value={note} onChange={e => setNote(e.target.value)} />
        </div>

        <button className="btn btn-red" onClick={handlePlaceOrder} disabled={placing}>
